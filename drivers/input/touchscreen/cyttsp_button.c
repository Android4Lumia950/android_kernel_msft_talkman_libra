...
#include "synaptics_dsx/synaptics_dsx_core.h"
#endif
...
static DEVICE_ATTR(rawdata, S_IRUSR, cyttsp_rawdata_show, NULL);
...
static int cyttsp_initialize_regulator(struct cyttsp_button_data *data)
{
	...
	ret = regulator_set_voltage(data->regulator_vdd, 1800000U, 1800000U);
	if (ret < 0) {
		dev_err(&client->dev,
			"%s: regulator_set_voltage vdd failed: %d\n",
			__func__, ret);
